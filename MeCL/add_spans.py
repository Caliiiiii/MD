...
def head_to_adj_oneshot(heads, sent_len, aspect_dict,
    ...
    for idx, head in enumerate(heads):
        if head != -1:
            if leaf2root:
                adj_matrix[head, idx] = 1  # -- head,idx
            if root2leaf:
                adj_matrix[idx, head] = 1  # -- idx,head

        if self_loop:
            adj_matrix[idx, idx] = 1  # -- idx,idx
...
def get_conditional_adj(father, length, cd_span,
    ...
    for i in range(len(s_slist) - 1):
        idx = s_slist[i]  # - 0
        begin_idx = cd_span.index(idx)  # - 0
        end_idx = len(cd_span) - cd_span[::-1].index(idx)  

        for j in range(idx + 1, len(s_slist)):  
            jdx = s_slist[j]  # - 2
            begin_jdx = cd_span.index(jdx)  # - 24
            end_jdx = len(cd_span) - cd_span[::-1].index(jdx)  # - 34
            for w_i in range(begin_idx, end_idx):  # - range(0,23)
                for w_j in range(begin_jdx, end_jdx):  # - range(24,34)
                    st_adj[w_i][w_j] = 0
                    st_adj[w_j][w_i] = 0
    return st_adj
...
def find_inner_LCA(path_dict, aspect_range):
    ...
    for idx in range(len(path_range[0])):
        flag = True
        for pid in range(1, len(path_range)):  # - len = 1 结果为空
...
def form_aspect_related_spans(aspect_node_idx, spans, mapnode, node2layerid, path_dict,
    ...
    aspect2root_path = path_dict[aspect_node_idx]  # - [2, 1, 0, -1]
    span_indications = []
    spans_range = []
    ...
    for idx, f in enumerate(aspect2root_path[:-1]):  
        if mapnode[f][:-len(special_token)] in select_N:  
            ...
            if len(spans_range) == 0 or span_temp != spans_range[-1]:  
                ...
                # - 'S'
    return spans_range, span_indications  # - ['NP', 'S']


def select_func(spans, max_num_spans, length):
    if len(spans) <= max_num_spans:  # - 2 <= 3
