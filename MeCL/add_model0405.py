...
class CLModel(nn.Module):
    ...
    def forward(self, inputs):
        ...
        out = self.fc(final)  # [batch_size, num_classes] -- (16,2)
        return out, h_t, h_b
...
class Model(nn.Module):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, (nn.Linear, nn.Embedding)):
            ...
            module.bias.data.zero_()
            module.weight.data.fill_(1.0)
        # is not None 相当于判断bias not False
        if isinstance(module, nn.Linear) and module.bias is not None:
            module.bias.data.zero_()  # -- initializes all the biases to zero
    ...
    def forward(self, inputs):
        ...
        embed_l = self.dropout1(embed_l)  # [batch_size, max_left_len, embed_dim]
        # - (64,90,768)
        embed_r = self.dropout2(embed_r)  # [batch_size, max_left_len, embed_dim] -- 仍然(16,70,768)
