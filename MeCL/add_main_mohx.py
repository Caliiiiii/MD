...
from add_model0405 import Model
from add_data_loader import dataset,collate_fn
from add_data_process import Verb_Processor
...
def get_kfold_data(k, i, raw_data):
    fold_size = len(raw_data) // k # len(raw_data) = 647, k = 10 , fold_size = 64
    ...
    return tr_raw, val_raw # 训练集,验证集


def main(args):
    os.environ['CUDA_VISIBLE_DEVICES'] = args.gpu
    # args.TRAIN.output -- './data/logs'
    # args.log -- mohx
    ...
    for i in range(10): 
        ...
        train_data, test_data = get_kfold_data(10, i, data) # 将验证集作为测试集
        train_set = dataset(train_data) # -- load data
        ...
        optimizer_grouped_parameters = [
            {
                # 不在no_dacay中的 就进行weight_decay 为0.01
