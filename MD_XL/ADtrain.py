...
class TrainerMbert(object):
    ...
    def preprocess_function(self, examples):
        return self.tokenizer(examples["text_a"], examples["text_b"], padding=True,truncation=True,max_length=150) # xlm-r
    ...
    def train_madx(self):
        ...
        model = AutoAdapterModel.from_pretrained(self.checkpoint,config=config)
        
        # load the language adapters
        lang_adapter_config = AdapterConfig.load("pfeiffer",reduction_factor=2)
        # model.load_adapter("en/wiki@ukp",config=lang_adapter_config)
        # model.load_adapter("./en_wiki_mbert",config=lang_adapter_config)
        model.load_adapter("./en_wiki_xlmr",config=lang_adapter_config)
        # model.load_adapter("zh/wiki@ukp",config=lang_adapter_config)
        # model.load_adapter("./zh_wiki_mbert",config=lang_adapter_config)
        model.load_adapter("./zh_wiki_xlmr",config=lang_adapter_config)

        # add a new task adapter
        model.add_adapter("meta")
        model.add_classification_head("meta",num_labels=2)
        
        model.train_adapter("meta")
        
        # unfreeze and activate stack setup
        model.active_adapters = Stack("en","meta")
        
        # set up trainer: 
        training_args = TrainingArguments(learning_rate=1e-4, num_train_epochs=3,
        ...
        trainer = AdapterTrainer(model=model, args=training_args, train_dataset=tokenized_dataset,
                          tokenizer=self.tokenizer,
                          data_collator=data_collator,
                          )
        # train:
        ...
        # --- 开始cross lingual transfer ----------
        
        # preprocess evaluation data:
        tokenized_dataset_target = self.preprocess_data(self.input_test)
    
        model.active_adapters = Stack("zh", "meta")
    
        # # perform zero-shot evaluation:
        # eval_args = TrainingArguments(learning_rate=1e-4, num_train_epochs=10,
        #                          # per_device_train_batch_size=32,
        #                          # per_device_eval_batch_size=32,
        #                          # logging_steps=100,
        #                          # output_dir="./training_output",
        ...
        #                          # remove_unused_columns = False
        #                          )
        ad_evaltrainer = AdapterTrainer(model=model, args=TrainingArguments(output_dir="./eval_output"),
                                        eval_dataset=tokenized_dataset_target,
                                        tokenizer=self.tokenizer,
                                       data_collator=data_collator)
        
        predictions = ad_evaltrainer.predict(tokenized_dataset_target)
    ...
    def mad_x(self, language, path_task_adapter): 
        ...
        if language == "zh":
            model.active_adapters = Stack("zh", adapter_name)
        # elif language == "ge":
